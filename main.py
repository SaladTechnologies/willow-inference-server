...
if device == "cuda":
    ...
    for cuda_dev_num in range(0, cuda_num_devices, 1):
        ...
        if cuda_free_memory <= settings.tts_memory_threshold:
            logger.warning(
                f"CUDA: Device {cuda_dev_num} has low memory, disabling TTS support"
            )
            support_tts = False
        ...
        if cuda_device_capability < 70 and support_chatbot:
            logger.warning(
                f"CUDA: Device {cuda_dev_num} is pre-Volta, disabling chatbot"
            )
            support_chatbot = False

        if cuda_free_memory <= 12000000000 and support_chatbot:
            logger.warning(
                f"CUDA: Device {cuda_dev_num} has low memory, disabling chatbot support"
            )
            support_chatbot = False
...
class LazyModels:
    ...
    def tts_processor(self):
        ...
        if self._tts_processor is None:
            self._tts_processor = transformers.SpeechT5Processor.from_pretrained(
    ...
    def chatbot_tokenizer(self):
        ...
        if self._chatbot_tokenizer is None:
            ...
            self._chatbot_tokenizer = AutoTokenizer.from_pretrained(
                chatbot_model_path, use_fast=True
            )
    ...
    def chatbot_model(self):
        ...
        if self._chatbot_tokenizer is None:
            logger.info(
                f"CHATBOT: Using model {chatbot_model_path} and CUDA, attempting load (this takes a while)..."
            )
...
ASSISTANT:"""
        logger.debug(
            f"CHATBOT: Pipeline parameters are max_new_tokens {max_new_tokens} temperature {temperature}"
...
def do_whisper(
    ...
    for i, mel_features_batch in enumerate(
        ...
        logger.debug(
            "Processing GPU batch %s of expected %s",
            i + 1,
...
def do_tts(text, format="WAV", speaker=tts_default_speaker):
    ...
    infer_time_milliseconds = infer_time.total_seconds() * 1000
    logger.debug(
        "TTS: Loading speaker embedding took " + str(infer_time_milliseconds) + " ms"
    )

    # Get inputs
    time_start = datetime.datetime.now()
    inputs = models.tts_processor(
        text=text, is_split_into_words=True, return_tensors="pt"
    ).to(device=device)
    time_end = datetime.datetime.now()
    infer_time = time_end - time_start
    infer_time_milliseconds = infer_time.total_seconds() * 1000
    ...
    with torch.inference_mode():
        audio = models.tts_model.generate_speech(
            inputs["input_ids"], speaker_embedding, vocoder=models.tts_vocoder
        ).to(device=device)
    time_end = datetime.datetime.now()
    infer_time = time_end - time_start
    infer_time_milliseconds = infer_time.total_seconds() * 1000
...
def do_speaker_embed(audio_file, speaker_name):
    ...
    try:
        classifier = EncoderClassifier.from_hparams(
            source=spk_model, run_opts={"device": device}, savedir=tmpdir
        )
...
app.mount(
    "/chatbot",
    StaticFiles(directory="nginx/static/chatbot", html=True),
    name="chatbot_files",
)
...
async def asr(
    ...
    (
        ...
        audio_file, model, beam_size, task, detect_language, force_language, translate
    )
...
if support_chatbot:
    ...
    @app.get(
        "/api/chatbot",
        summary="Submit text for chatbot",
        response_description="Chatbot answer",
    )
    async def chatbot(
        text: str,
        max_new_tokens: Optional[int] = chatbot_max_new_tokens,
        temperature: Optional[float] = chatbot_temperature,
        top_p: Optional[float] = chatbot_top_p,
        repetition_penalty: Optional[float] = chatbot_repetition_penalty,
    ):
        logger.debug(f"FASTAPI: Got chatbot request with text: {text}")
        # Do Chatbot
        response = do_chatbot(
            text, max_new_tokens, temperature, top_p, repetition_penalty
        )
    ...
    @app.get(
        "/api/chatbot/tts",
        summary="Submit text for chatbot and get audio in response",
        response_description="Chatbot answer audio",
    )
    async def chatbot_tts(
        text: str,
        max_new_tokens: Optional[int] = chatbot_max_new_tokens,
        temperature: Optional[float] = chatbot_temperature,
        top_p: Optional[float] = chatbot_top_p,
        ...
        logger.debug(
            f"FASTAPI: Got chatbot TTS request with text: {text} and speaker {speaker}"
        )
        # Do Chatbot
        chatbot = do_chatbot(
            text, max_new_tokens, temperature, top_p, repetition_penalty
...
if support_tts:
    ...
    @app.get(
        "/api/tts",
        summary="Submit text for text to speech",
    ...
    async def tts(
        ...
        logger.debug(
            f"FASTAPI: Got TTS request for speaker {speaker} with format {format} and text: {text}"
        )
        # Do TTS
    ...
    @app.post(
        "/api/sts",
        summary="Submit speech, do ASR, and TTS",
        response_description="Audio file of generated speech",
    )
    async def sts(
        request: Request,
        ...
        logger.debug(
            f"FASTAPI: Got STS request for model {model} beam size {beam_size} "
            f"language detection {detect_language}"
        )
        task = "transcribe"
    ...
    @app.post(
        ...
        summary="Add custom speaker",
        response_description="Speaker creation status",
    )
    async def speaker_create(
        request: Request, audio_file: UploadFile, speaker_name: Optional[str] = "CUSTOM"
    ):
    ...
    @app.get(
        "/api/speaker",
        response_model=SpeakersList,
        summary="Show supported speakers",
